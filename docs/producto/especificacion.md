...
> Estado: **v2 en diseño** · v1 (MVP, F0–F8) construida · Ampliación v2 añadida el 2026-09-24, pendiente de arquitectura
...
| Fase | Contenido | Qué riesgo reduce o qué enseña |
|---|---|---|
| **F9 — Esqueleto vertical de la v2** *(desechable)* | Un solo camino de punta a punta con toda la infraestructura nueva: un endpoint encola un trabajo → el `worker` genera un PDF trivial → lo guarda en el almacén de ficheros → envía por email (Mailpit) un aviso con el enlace de descarga. Sin interfaz bonita ni reglas de negocio. | Descubre dónde duele antes de construir encima: dependencias de sistema del generador de PDF dentro de la imagen, cola y worker en Docker, almacén compatible con S3, SMTP. Lo más probable que falle, y lo más caro de descubrir a mitad de una feature. |
...
| R9 | El generador de PDF necesita librerías de sistema (tipografías, renderizado) que pueden no estar en la imagen o comportarse distinto en producción. | Medio. | Lo descubre F9 a propósito. |
